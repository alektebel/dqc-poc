...
def _run_rule_pipeline(entry: dict, fields: list, table_name: str,
                       cases, *, value_grounding: bool = False,
    ...
    for intento in range(1, react.MAX_ATTEMPTS + 1):
        ...
        if not val_errors and cases is not None:
            ...
            if not run["ok"]:
                ...
                m = react.metrics(cases, run["casos"], entry.get("prev_id"))
                if m:
                    validacion.update(m)
                    out.metrics = m
        trace.append({"paso": "validacion",
...
async def generate_dqc_stream(
    ...
    def event_stream():
        ...
        for entry in plan:
            eid = entry["id"]
            gen = _run_rule_pipeline(
                entry, fields, table_name, cases,
