...
from api.dq.pipeline import (  # the agent loop, now invocable without HTTP
    DQCItem,
    parse_dqc_items as _parse_dqc_items,
    run_rule_pipeline,
)
...
async def inspect_dictionary(
    ...
    _require_tabular(dictionary)
    raw = await dictionary.read()
    try:
        inspection = dict_ai.inspect_workbook(raw, dictionary.filename)
...
def _require_tabular(upload: UploadFile, what: str = "dictionary") -> None:
    """Accept the two formats the reader understands: .xlsx and .csv.

    The legacy .xls is named explicitly because openpyxl cannot read it
    and the generic message would send the user looking for the wrong
    problem.
    """
    ...
    if not name.endswith((".xlsx", ".xls", ".csv")):
        ...
        raise HTTPException(
            status_code=400,
            detail=f"{what} debe ser un fichero .xlsx o .csv")


def _workbook_read_error(filename: str | None, exc: Exception) -> str:
    """User-facing detail for a file the reader could not load."""
    name = (filename or "").lower()
    if name.endswith(".xls"):
        return ("El formato .xls antiguo no está soportado. Abre el fichero "
                "en Excel, guárdalo como .xlsx (o .csv) y vuelve a subirlo.")
    return (f"No se pudo leer el fichero ({exc.__class__.__name__}). "
            "Comprueba que es un .xlsx válido o un .csv con cabeceras.")
...
def _resolve_dictionary_context(
    ...
    if not sheet:
        try:
            inspection = dict_ai.inspect_workbook(raw, filename)
...
# ── Per-rule agent loop (shared by the SSE stream and the revisions API) ────
    ...
    if data_file is not None and data_file.filename:
        ...
        try:
            cases = react.load_cases(data_bytes, table_name,
                                     data_file.filename)
            executor = UploadedTableExecutor(cases, table_name)
    ...
    def event_stream():
        ...
        for entry in plan:
            gen = run_rule_pipeline(
                entry, fields, table_name, executor, client=get_client(),
                value_grounding=value_grounding, semantic_judge=semantic_judge)
            outcome = None
            while True:
                try:
                    yield _sse("item", next(gen))
            ...
            if outcome.estado == "ambigua":
                ambiguas += 1
                continue
            if outcome.estado == "error":
                errors.append(outcome.error)
                continue
            if outcome.executed:
                comprobados += 1
            if outcome.metrics:
                precisions.append(outcome.metrics["precision"])
                recalls.append(outcome.metrics["recall"])
            for it in outcome.items:
                if outcome.validacion and outcome.validacion.get("ejecutada"):
                    validaciones[id(it)] = outcome.validacion
                traces[id(it)] = outcome.trace
                if outcome.atribucion:
                    atribuciones[id(it)] = outcome.atribucion
            dqcs.extend(outcome.items)
...
async def evaluate_checks(
    ...
    _require_tabular(data_file, what="data_file")
    raw = await data_file.read()
    try:
        cases = react.load_cases(raw, table_name, data_file.filename)
    except Exception as exc:  # noqa: BLE001 — unreadable workbook is a user error
        raise HTTPException(
            status_code=400,
            detail=_workbook_read_error(data_file.filename, exc))
